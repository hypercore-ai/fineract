...
import org.apache.fineract.portfolio.loanaccount.loanschedule.exception.ActiveOnApprovalException;
...
public abstract class AbstractLoanScheduleGenerator implements LoanScheduleGenerator {
    ...
    private List<DisbursementData> loanDisbursements;
    ...
    private LoanScheduleModel generate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if (!activeLoanWithoutDisbursementSchedule) {
            while (!scheduleParams.getOutstandingBalance().isZero() || !scheduleParams.getDisburseDetailMap().isEmpty()) {
                ...
                if (loanApplicationTerms.isMultiDisburseLoan()) {
                    ...
                    updateEMIorPrincipalPaymentForMultiDisbursement(mc, loanApplicationTerms, scheduleParams,
                            isBalanceChangedByDisbursement);
                }
                ...
                handleRecalculationForNonDueDateTransactions(mc, loanApplicationTerms, loanCharges, holidayDetailDTO, scheduleParams,
                        periods, loanApplicationTerms.getTotalInterestDue(), idealDisbursementDate, firstRepaymentdate, lastRestDate,
                        scheduledDueDate, periodStartDateApplicableForInterest, applicableTransactions, currentPeriodParams);
                ...
                TreeMap<LocalDate, Money> principalVariations = mergeVariationsToMap(scheduleParams);
                ComponentsForPeriod componentsForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                        ...
                        loanApplicationTerms, scheduleParams.getPeriodNumber(), mc, principalVariations, scheduleParams.getCompoundingMap(),
                ...
                if (loanApplicationTerms.getFixedEmiAmount() != null
                        && loanApplicationTerms.getFixedEmiAmount().compareTo(componentsForThisPeriod.interest().getAmount()) < 0
                        && !loanApplicationTerms.isInterestToBeAppropriatedEquallyWhenGreaterThanEMIEnabled()) {
                    String errorMsg = "EMI amount must be greater than : " + componentsForThisPeriod.interest().getAmount();
                    throw new MultiDisbursementEmiAmountException(errorMsg, componentsForThisPeriod.interest().getAmount(),
                            loanApplicationTerms.getFixedEmiAmount());
                }

                // update cumulative fields for principal & interest
                currentPeriodParams.setInterestForThisPeriod(componentsForThisPeriod.interest());
                Money lastTotalOutstandingInterestPaymentDueToGrace = scheduleParams.getTotalOutstandingInterestPaymentDueToGrace();
                scheduleParams.setTotalOutstandingInterestPaymentDueToGrace(componentsForThisPeriod.interestPaymentDueToGrace());
                currentPeriodParams.setPrincipalForThisPeriod(componentsForThisPeriod.principal());
                ...
                installment = handleRecalculationForTransactions(mc, loanApplicationTerms, holidayDetailDTO, currency, scheduleParams,
                        loanRepaymentScheduleTransactionProcessor, loanApplicationTerms.getTotalInterestDue(), lastRestDate,
                        scheduledDueDate, periodStartDateApplicableForInterest, applicableTransactions, currentPeriodParams,
            ...
            if (!loanApplicationTerms.isActivatedOnApproval()) {
                throw new ActiveOnApprovalException("Schedule with no disbursement only valid for active on approval products");
            }

            while (scheduleParams.getInstalmentNumber() <= numberOfRepayments) {
                LocalDate previousRepaymentDate = scheduleParams.getActualRepaymentDate();
                scheduleParams.setActualRepaymentDate(this.scheduledDateGenerator
                        .generateNextRepaymentDate(scheduleParams.getActualRepaymentDate(), loanApplicationTerms, isFirstRepayment));
                AdjustedDateDetailsDTO adjustedDateDetailsDTO = this.scheduledDateGenerator
        ...
        // loan interest
        updatePeriodsWithCharges(currency, scheduleParams, periods, nonCompoundingCharges, loanApplicationTerms, mc);
    ...
    private void applyChargesForCurrentPeriod(final Set<LoanCharge> loanCharges, final MonetaryCurrency currency,
        ...
        ComponentsForPeriod principalInterest = new ComponentsForPeriod(currentPeriodParams.getPrincipalForThisPeriod(),
        ...
        currentPeriodParams.setFeeChargesForInstallment(cumulativeFeeChargesDueWithin(scheduleParams.getPeriodStartDate(), scheduledDueDate,
                ...
                scheduleParams.getTotalCumulativeInterest(), true, loanApplicationTerms, scheduleParams.getLoanTransactions(), mc, scheduleParams));
    ...
    private void updatePeriodsWithCharges(final MonetaryCurrency currency, LoanScheduleParams scheduleParams,
        ...
        for (LoanScheduleModelPeriod loanScheduleModelPeriod : periods) {
            if (loanScheduleModelPeriod.isRepaymentPeriod()) {
                ComponentsForPeriod principalInterest = new ComponentsForPeriod(Money.of(currency, loanScheduleModelPeriod.principalDue()),
                ...
                Money feeChargesForInstallment = cumulativeFeeChargesDueWithin(loanScheduleModelPeriod.periodFromDate(),
                        ...
                        !loanScheduleModelPeriod.isRecalculatedInterestComponent(), loanApplicationTerms,
                        scheduleParams.getLoanTransactions(), mc, scheduleParams);
    ...
    private LoanScheduleModelPeriod handlePrepaymentOfLoan(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        ComponentsForPeriod tempPrincipalInterest = new ComponentsForPeriod(currentPeriodParams.getPrincipalForThisPeriod(),
                currentPeriodParams.getInterestForThisPeriod(), null);
        ...
        if (!oustanding.isGreaterThan(currentPeriodParams.getInterestForThisPeriod()) && !scheduledDueDate.equals(transactionDate)) {
            ...
            ComponentsForPeriod interestTillDate = calculatePrincipalInterestComponentsForPeriod(this.paymentPeriodsInOneYearCalculator,
            ...
            applyChargesForCurrentPeriod(loanCharges, totalInterestChargedForFullLoanTerm.getCurrency(), scheduleParams, calculateTill,
                    tempPeriod, loanApplicationTerms, mc);
                    ...
                    if (scheduleParams.getLoanRepaymentScheduleTransactionProcessor() != null && scheduleParams
                        ...
                        if (!transactionDate.isEqual(scheduleParams.getPeriodStartDate()) || scheduleParams.getInstalmentNumber() == 1) {
                            ...
                            if (daysInPeriodApplicable > 0) {
                                ...
                                // date
                                ComponentsForPeriod principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                            ...
                            // newly
                            // created installment
                            ComponentsForPeriod principalInterest = new ComponentsForPeriod(principalForThisPeriod,
                            ...
                            Money feeChargesForInstallment = cumulativeFeeChargesDueWithin(scheduleParams.getPeriodStartDate(),
                                    ...
                                    scheduleParams.getTotalCumulativeInterest(), false, loanApplicationTerms,
                                    scheduleParams.getLoanTransactions(), mc, scheduleParams);
            ...
            while (compoundingDate.isBefore(endDate)) {
                ...
                if (compoundingDate.isBefore(endDate)) {
                    Money feeChargesForInstallment = cumulativeFeeChargesDueWithin(lastCompoundingDate, compoundingDate, charges, currency,
                            null, loanApplicationTerms.getPrincipal(), null, false, loanApplicationTerms,
                            scheduleParams.getLoanTransactions(), mc, scheduleParams);
                    Money penaltyChargesForInstallment = cumulativePenaltyChargesDueWithin(lastCompoundingDate, compoundingDate, charges,
                            currency, null, loanApplicationTerms.getPrincipal(), null, false, loanApplicationTerms,
                            scheduleParams.getLoanTransactions(), mc, scheduleParams);
    ...
    public abstract ComponentsForPeriod calculatePrincipalInterestComponentsForPeriod(PaymentPeriodsInOneYearCalculator calculator,
    ...
    private BigDecimal getDisbursementAmountAndUpdatePeriods(final LoanApplicationTerms loanApplicationTerms, LocalDate disbursementDate,
    ...
    private BigDecimal calcUntilizedChargeAmount(LoanApplicationTerms applicationTerms, final LoanCharge loanCharge, LocalDate periodStart,
                                                 LocalDate periodEnd, List<LoanTransaction> transactions, final MathContext mc, LoanScheduleParams scheduleParams) {
        ...
        Money loanPrincipal = approvedPrincipal != null && approvedPrincipal.isGreaterThanZero() ? approvedPrincipal
                : applicationTerms.getOriginalProposedPrincipal();

        final boolean isActiveLoan = scheduleParams.getLoanStatus() != null && scheduleParams.getLoanStatus().isActive();
        // Calc disbursed until start period
        BigDecimal disbursedAmount = this.loanDisbursements.stream()
                .filter(disbursementData -> isActiveLoan ?
                        disbursementData.getActualDisbursementDate() != null && disbursementData.getActualDisbursementDate().isBefore(periodStart) :
                        disbursementData.getExpectedDisbursementDate().isBefore(periodStart))
        ...
        Stream<TransactionHelper> disbursementsInPeriodStream = this.loanDisbursements.stream()
                .filter(disbursementData -> {
                    LocalDate disbursementDate = isActiveLoan ? disbursementData.getActualDisbursementDate() : disbursementData.getExpectedDisbursementDate();
                    return disbursementDate != null && CalendarUtils.checkDateBetween(disbursementDate, periodStart, periodEnd);
                })
        ...
        LocalDate revolvingPeriodEndDate = applicationTerms.getRevolvingPeriodEndDate();
        LocalDate revolvingPeriodStartDate = applicationTerms.getRevolvingPeriodStartDate();

        boolean inRevolvingPeriodOnly = loanCharge.isRevolvingPeriodInstalmentFee();
        if (inRevolvingPeriodOnly) {
            ...
            if (isRevolvingStartDateValid || isRevolvingEndDateValid) {
                if (transactions != null && !transactions.isEmpty()) {
                    MonetaryCurrency currency = applicationTerms.getCurrency();

                    BigDecimal totalUntilPeriodStartRepaid = transactions.stream()
                            .filter(loanTransaction -> loanTransaction.isPaymentTransaction()
                                    && loanTransaction.getTransactionDate().isBefore(periodStart))
                            .map(loanTransaction -> loanTransaction.getPrincipalPortion(currency).getAmount())
                            .reduce(BigDecimal.ZERO, BigDecimal::add);
                    ...
                    transactionInPeriodStream = transactions.stream()
                            ...
                            .map(loanTransaction -> new TransactionHelper(loanTransaction.getTransactionDate(),
                                    loanTransaction.getPrincipalPortion(currency).getAmount()));
                }
            } else {
    ...
    private Money cumulativeFeeChargesDueWithin(final LocalDate periodStart, final LocalDate periodEnd, final Set<LoanCharge> loanCharges,
                                                final MonetaryCurrency monetaryCurrency, final ComponentsForPeriod principalInterestForThisPeriod,
                                                final Money principalDisbursed, final Money totalInterestChargedForFullLoanTerm, boolean isInstallmentChargeApplicable,
                                                final LoanApplicationTerms loanApplicationTerms, List<LoanTransaction> transactions, final MathContext mc, LoanScheduleParams scheduleParams) {
            ...
            if (!loanCharge.isDueAtDisbursement() && loanCharge.isFeeCharge()) {
                if ((loanCharge.isInstalmentFee() || loanCharge.isRevolvingPeriodInstalmentFee()) && isInstallmentChargeApplicable) {
                    cumulative = calculateInstallmentCharge(principalInterestForThisPeriod, cumulative, loanCharge, loanApplicationTerms,
                            periodStart, periodEnd, transactions, mc, scheduleParams);
    ...
    private Money calculateInstallmentCharge(final ComponentsForPeriod principalInterestForThisPeriod, Money cumulative,
                                             final LoanCharge loanCharge, LoanApplicationTerms loanApplicationTerms, LocalDate periodStart, LocalDate periodEnd,
                                             List<LoanTransaction> transactions, final MathContext mc, LoanScheduleParams scheduleParams) {
        if (loanCharge.getChargeCalculation().isPercentageBased()) {
            ...
            if (loanCharge.getChargeCalculation().isPercentageOfUnutilizedAmount()) {
                loanChargeAmt = calcUntilizedChargeAmount(loanApplicationTerms, loanCharge, periodStart, periodEnd, transactions, mc, scheduleParams);
            } else {
    ...
    private Money cumulativePenaltyChargesDueWithin(final LocalDate periodStart, final LocalDate periodEnd,
                                                    ...
                                                    final ComponentsForPeriod principalInterestForThisPeriod, final Money principalDisbursed,
    ...
    private LoanScheduleDTO rescheduleNextInstallments(final MathContext mc, final LoanApplicationTerms loanApplicationTerms, Loan loan,
        ...
        if (rescheduleFrom != null) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                // tranche
                // details to map
                BigDecimal disburseAmt = getDisbursementAmountAndUpdatePeriods(loanApplicationTerms,
                        loanApplicationTerms.getExpectedDisbursementDate(), periods, chargesDueAtTimeOfDisbursement, disburseDetailMap,
                        true);
            ...
            if (!newRepaymentScheduleInstallments.isEmpty() && totalCumulativeInterest.isGreaterThanZero()) {
                ...
                loanScheduleParams = LoanScheduleParams.createLoanScheduleParamsForPartialUpdate(periodNumber, instalmentNumber,
                        ...
                        currency, applyInterestRecalculation, transactions, loan.status());
        ...
        if (loanScheduleParams == null) {
            loanScheduleParams = LoanScheduleParams.createLoanScheduleParamsForCompleteUpdate(recalculationDetails,
                    loanRepaymentScheduleTransactionProcessor, scheduleTillDate, applyInterestRecalculation, transactions, loan.status());
            periods.clear();
        }
        ...
        boolean activeNotDisbursed = loan.shouldActivateOnApproval() && loan.isMultiDisburmentLoan() && !loan.isDisbursed();
    ...
    private LoanRepaymentScheduleInstallment addLoanRepaymentScheduleInstallment(final List<LoanRepaymentScheduleInstallment> installments,
        ...
        if (scheduledLoanInstallment.isRepaymentPeriod() || scheduledLoanInstallment.isRepaymentSubPeriod()) {
            installment = new LoanRepaymentScheduleInstallment(null, scheduledLoanInstallment.periodNumber(),
                    scheduledLoanInstallment.subPeriodNumber(), scheduledLoanInstallment.periodFromDate(),
